...
| **Neon（PostGIS）** | ✅ 実装済み・**Neon未検証** | ローカルの PostGIS で全機能の疎通を確認済み。Neon 実環境ではまだ流していない。`DATABASE_URL` を設定すれば実データ、未設定ならモックに切り替わる。**スキーマは実測（店舗特定率）の結果で変わりうる**ため、収集対象が変わったら見直すこと |
...
| 広告・アフィリエイト | ❌ 未着手 | 収益化はユーザーが付いてから（[実装ガイド](docs/8_広告・アフィリエイト実装ガイド.md)） |
...
| 10 | [資金調達の申請先まとめ](docs/10_資金調達の申請先まとめ.md) | 公庫・助成金・補助金・クラファンの比較と進め方 |
| 11 | [補助金・融資の申請ドラフト](docs/11_補助金・融資の申請ドラフト.md) | 公庫の創業計画書・東京都創業助成・持続化補助金の原稿 |
